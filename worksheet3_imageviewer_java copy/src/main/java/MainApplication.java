            ...
            while(line != null)
            {
                if(line.trim().length() > 0) // Ignore blank lines
                {
                    ...
                    if(parts.length < 5)
                    {
                        ...
                        newRecord.addFilename(imageFilename);
                        newRecord.addCaption(imageCaption);
                        for (int i = 2; i < parts.length; i++) {
                            String[] labelParts = parts[i].split("=");
                            String label = labelParts[0];
                            String data = labelParts[1];

                            if (label.equals("rating")) {
                                newRecord = new Rating(newRecord, data);
                            }

                            if (label.equals("date")) {
                                newRecord = new Date(newRecord, data);
                            }

                            if (label.equals("gps")) {
                                newRecord = new GPS(newRecord, data);
                            }
                        }
                    }
                    
                    // FIXME: Insert your code here to add a new image to the album.
                    album.addImage(newRecord);
                }
